  ...
  try {
    ...
    if (result.success && result.data && result.data.articles && Array.isArray(result.data.articles)) {
      ...
      return posts.map((post: any) => ({
        ...
        category: post.category ? post.category.charAt(0).toUpperCase() + post.category.slice(1) : 'Gaming',
        author: post.author,
...
function PostCarousel({ articles }: { articles: NewsArticle[] }) {
    ...
    <div className="relative w-full h-96 overflow-hidden rounded-lg mb-8">
      ...
      >
        {articles.map((article, index) => (
          <div key={article.id} className="w-full h-full flex-shrink-0 relative">
            <Link href={`/article/news/${article.slug}`}>
              <div className="relative w-full h-full cursor-pointer bg-gray-200">
                {article.media.url && article.media.url !== "/placeholder.svg?height=300&width=400" ? (
                  <MediaDisplay
                    media={article.media}
                    title={article.title}
                ...
                <div className="absolute bottom-0 left-0 right-0 p-6 text-white">
                  <span className="bg-red-600 px-3 py-1 text-xs font-bold rounded mb-3 inline-block">
                    {article.category}
                  </span>
                  <h3 className="text-2xl font-bold mb-2 line-clamp-2">{article.title}</h3>
                  <p className="text-white/90 mb-4 line-clamp-2">{article.excerpt}</p>
    ...
    <div className="min-h-screen bg-white">
      ...
      <section className="px-4 lg:px-8 py-6">
        ...
        >
          ...
          <PostCarousel articles={newsArticles.slice(0, 5)} />
        </motion.div>
      </section>
