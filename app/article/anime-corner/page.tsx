...
async function fetchAnime() {
  try {
    const response = await fetch("http://localhost:3001/api/news?category=anime-corner&limit=10", {
    ...
    return [];
  } catch (error) { console.error("Failed to fetch anime:", error); return []; }
}
...
export default function AnimeCornerPage() {
  ...
  useEffect(() => {
    async function loadAnime() {
      const articles = await fetchAnime();
      setAnime(articles);
      setLoading(false);
    }
    loadAnime();
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-white">
        <section className="bg-gradient-to-r from-pink-600 to-pink-800 text-white py-8 lg:py-12">
          <div className="px-4 lg:px-8"><div className="animate-pulse">
            <div className="h-8 lg:h-12 bg-pink-500 rounded mb-2 lg:mb-4 w-64"></div>
            <div className="h-4 lg:h-6 bg-pink-500 rounded w-96"></div>
          </div></div>
        </section>
      </div>
    );
    ...
    <div className="min-h-screen bg-white">
      <section className="bg-gradient-to-r from-pink-600 to-pink-800 text-white py-8 lg:py-12">
        <div className="px-4 lg:px-8">
          <motion.h1 initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} className="text-2xl lg:text-4xl font-bold mb-2 lg:mb-4">Anime Corner</motion.h1>
          <p className="text-sm lg:text-xl opacity-90">Latest anime news, reviews, and discussions</p>
        </div>
      </section>
      <section className="px-4 lg:px-8 py-6 lg:py-8">
        {anime.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {anime.map((item, index) => (
              <motion.article key={item.id} initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.1 }} className="bg-white rounded-lg overflow-hidden shadow-lg hover:shadow-xl transition-shadow border">
                ...
                <div className="p-6">
                  <span className="text-pink-600 text-xs font-semibold uppercase">Anime</span>
                  <Link href={`/article/news/${item.slug}`}>
                    <h3 className="text-gray-900 font-bold text-lg leading-tight mt-2 mb-3 hover:text-pink-600 transition-colors cursor-pointer">{item.title}</h3>
                  </Link>
          ...
          <div className="text-center py-12">
            <h2 className="text-2xl font-bold text-gray-900 mb-4">No Anime Content Yet</h2>
            <p className="text-gray-600">Check back later for the latest anime updates!</p>
          </div>
        )}
      </section>
    </div>
