...
export default function GamingHubPage() {
  ...
  useEffect(() => {
    async function fetchArticles() {
      try {
        const response = await fetch('https://admin.koodos.in/api/public/articles?category=gaming-hub&status=PUBLISHED&limit=20')
        if (response.ok) {
          const data = await response.json()
          setArticles(data || [])
        }
      } catch (error) {
        console.error('Failed to fetch gaming hub articles:', error)
      } finally {
        setLoading(false)
      }
    }
    ...
    return (
      <div className="min-h-screen bg-white">
        <div className="max-w-6xl mx-auto px-6 py-8">
          <h1 className="text-4xl font-bold mb-8 text-gray-900">Gaming Hub</h1>
      ...
      <div className="max-w-6xl mx-auto px-6 py-8">
        ...
        {articles.length === 0 ? (
          <div className="text-center py-12">
            <p className="text-gray-600">No gaming hub articles available yet.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {articles.map((article, index) => (
              <Link key={article.id} href={`/article/gaming-hub/${article.slug}`}>
                <motion.div
