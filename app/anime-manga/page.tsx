...
import { Star, Users, Calendar, Camera, Palette, BookOpen, Sparkles, ArrowRight, TrendingUp, Eye, Clock, MessageCircle, ThumbsUp, Play } from 'lucide-react'
                            ...
                            <div className="absolute top-4 right-4">
                              <div className="px-4 py-2 rounded-xl border-2 bg-white/90 border-pink-300 text-pink-600">
                                <div className="flex items-center space-x-1">
                                  <Star className="w-4 h-4" /> <span>{review.rating}</span>
                                </div>
                              </div>
                            </div>
                            <div className="absolute bottom-4 left-4 flex gap-2">
                              {review.trending && <Badge className="bg-red-600 text-white font-bold px-3 py-1">Trending</Badge>}
                            </div>
                          </div>
                        </div>
                        ...
                        <CardContent className="lg:col-span-2 p-6 lg:p-8">
                          <div className="flex items-start justify-between mb-4">
                            ...
                            <div className="flex items-center space-x-4 text-sm text-gray-500">
                              <div className="flex items-center space-x-1">
                                <Calendar className="w-4 h-4" /> <span>{review.publishDate}</span>
                              </div>
                              <div className="flex items-center space-x-1">
                                <Clock className="w-4 h-4" /> <span>{review.readTime}</span>
                              </div>
                            </div>
                          </div>
                          ...
                          <div className="flex flex-wrap gap-2 mb-4">
                            {review.tags.map(tag => <span key={tag} className="text-xs bg-gray-200 text-gray-800 px-2 py-1 rounded-full">{tag}</span>)}
                          </div>
                          ...
                          <div className="flex items-center justify-between">
                            <div className="flex items-center space-x-6 text-sm text-gray-500">
                              <div className="flex items-center space-x-2">
                                <Eye className="w-4 h-4" /> <span>{review.views}</span>
                              </div>
                              <div className="flex items-center space-x-2">
                                <ThumbsUp className="w-4 h-4" /> <span>{review.likes}</span>
                              </div>
                              <div className="flex items-center space-x-2">
                                <MessageCircle className="w-4 h-4" /> <span>{review.comments}</span>
                              </div>
                            </div>
                          </div>
                        </CardContent>
                        ...
                        <div className="flex items-center space-x-3">
                          ...
                          <div>
                            ...
                            <div className="text-xs text-gray-500">
                              {manga.weeklyReads} reads
                            </div>
                          </div>
                        </div>
                        <div className="flex items-center space-x-1 text-pink-600">
                          <Star className="w-4 h-4" /> <span>{manga.rating}</span>
                        </div>
                      </div>
                    </Link>
                  ))}
                  ...
                  <Card className="group hover:shadow-2xl transition-all duration-500 cursor-pointer border border-gray-200 hover:border-purple-200 overflow-hidden">
                      ...
                      <div className="aspect-square bg-gradient-to-br from-purple-100 to-pink-100 relative">
                        <div className="absolute inset-0 bg-gradient-to-br from-purple-500/20 to-pink-500/20 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                          <div className="w-16 h-16 bg-white/90 rounded-full flex items-center justify-center group-hover:scale-110 transition-transform duration-300">
                            {cosplay.category === 'Tutorial' ? <Play className="w-8 h-8 text-purple-600" /> : <Camera className="w-8 h-8 text-purple-600" />}
                          </div>
                        </div>
                        <div className="absolute top-4 left-4 flex gap-2">
                          {cosplay.featured && <Badge className="bg-purple-600 text-white text-xs font-medium">Featured</Badge>}
                        </div>
                        <div className="absolute top-4 right-4">
                          <div className="flex items-center space-x-1 bg-black/70 text-white px-2 py-1 rounded text-xs">
                            <Users className="w-3 h-3" /> <span>{cosplay.cosplayer}</span>
                          </div>
                        </div>
                    ...
                    <CardContent className="p-6">
                      ...
                      <div className="flex flex-wrap gap-1 mb-3">
                        {cosplay.materials.map(material => <span key={material} className="text-xs bg-gray-200 text-gray-800 px-2 py-1 rounded-full">{material}</span>)}
                      </div>

                      <div className="flex items-center justify-between pt-3 border-t border-gray-100">
                        <div className="flex items-center space-x-3 text-xs text-gray-500">
                          <div className="flex items-center space-x-1">
                            <ThumbsUp className="w-3 h-3" /> <span>{cosplay.likes}</span>
                          </div>
                          <div className="flex items-center space-x-1">
                            <MessageCircle className="w-3 h-3" /> <span>{cosplay.comments}</span>
                          </div>
                        </div>
                      </div>
                    </CardContent>
