...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({ error: 'Failed to process user data' }, { status: 500, headers: getCorsHeaders(origin) })
  }
}

export async function GET(request: NextRequest) {
  const origin = request.headers.get('origin');
  return NextResponse.json({ message: 'User data endpoint' }, { headers: getCorsHeaders(origin) })
}